...
/**
 * Debug class for AggregateStats to print exact values
 */
public class AggregateStatsDebug {
  @Test
  public void debugAllTests() throws Exception {
    System.out.println("\n--- DEBUG: testAggregationsStats ---");
    debugAggregationsStats();
    
    System.out.println("\n--- DEBUG: testByteAndTimeTotalAggregation ---");
    debugByteAndTimeTotalAggregation();
    
    System.out.println("\n--- DEBUG: testByteAndTimeAverageAggregation ---");
    debugByteAndTimeAverageAggregation();
    
    System.out.println("\n--- DEBUG: testDifferentUnits ---");
    debugDifferentUnits();
    
    System.out.println("\n--- DEBUG: testMixedInputUnits ---");
    debugMixedInputUnits();
  }
  
  private void debugAggregationsStats() throws Exception {
    ...
    Double totalSize = (Double) lastRow.getValue("total_size");
    Double totalTime = (Double) lastRow.getValue("total_time");
    System.out.println("total_size: " + totalSize);
    System.out.println("total_time: " + totalTime);
  }

  private void debugByteAndTimeTotalAggregation() throws Exception {
    String[] directives = new String[] {
  ...
  private void debugByteAndTimeAverageAggregation() throws Exception {
    String[] directives = new String[] {
    ...
    System.out.println("Result size: " + results.size());
    Row lastRow = results.get(results.size() - 1);
    Double avgSize = (Double) lastRow.getValue("avg_size");
    Double avgTime = (Double) lastRow.getValue("avg_time");
    System.out.println("avg_size: " + avgSize);
    System.out.println("avg_time: " + avgTime);
  }
  
  private void debugDifferentUnits() throws Exception {
    String[] directives = new String[] {
  ...
  private void debugMixedInputUnits() throws Exception {
    String[] directives = new String[] {
      "aggregate-stats :data_size :response_time :total_size :total_time MB s total"
    };
